...
- **Tests / CI** — 63 Rust unit tests; GitHub Actions runs **21 gated jobs** (rust test + `clippy -D warnings`, kernel/ISO build, alt-config matrix, fourteen QEMU self-tests — smoke-boot, ELF/W^X, preemption, signals, process-control, async notifications, SMP, and the filesystem/libc suite: fs, fs-perms, fs-conc, fs-persist, fs-wal, fs-large, newlib — the scripted `smoke-session` integration test, a reproducible-build check, a security scan + SBOM, and a TLA+ model-checking job that TLC-verifies the capability-algebra subset-rights and paging-isolation invariants).

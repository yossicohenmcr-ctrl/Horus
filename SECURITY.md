...
- **SMP capability-read integrity:** the lock-free `cap_lookup` is a seqlock reader over `cap_seq` (bumped odd on every `cap_lock` acquire, even on release), so a capability lookup on one CPU can never observe a torn `capability_t` (e.g. new rights with an old serial) or a half-nulled slot while a mint/transfer/revoke-sweep/grant mutates cspaces under `cap_lock` on another CPU. In-lock callers (`kcap_lookup`, `task_kill_authorized`, `SYS_CAP_GRANT`) use a non-retrying `cap_lookup_locked` so nesting a lookup inside a `cap_lock` critical section cannot self-deadlock. The seqlock protocol is TLA+ model-checked (`docs/cap_seqlock.tla`, `tla` CI job): TLC exhaustively verifies that a committed lock-free read never observes a torn capability across all two-CPU interleavings.
...
- **Supply chain / CI:** every change is gated by a pipeline of 22 jobs — `cargo test`, `clippy` with all warnings denied, a kernel + ISO build, an alt-config build matrix (`DEBUG_SHELL=1`, `MINIMAL_SECURE=1`), fifteen headless QEMU self-tests (smoke-boot, ELF-loader + W^X, preemption, signals, process-control, async notifications, SMP, the six-part encrypted-filesystem/libc suite, and an Ed25519 verified-boot gate that authorizes boot on a valid signed manifest and halts on a tampered one), a scripted `smoke-session` integration test that drives the real ring-3 shell over serial, a byte-for-byte reproducible-build check, a security-scan job (Semgrep, Trivy, gitleaks, cppcheck, flawfinder, `cargo-audit`) that also emits a CycloneDX SBOM, and a TLA+ model-checking job (TLC exhaustively verifies the capability-algebra subset-rights and paging-isolation invariants). gitleaks (committed secrets, full-history), Semgrep ERROR-severity findings, and a violated TLA+ invariant are **blocking gates**; the remaining scanners are advisory. The `74`/`22` counts are enforced by `tools/check_doc_counts.sh` (see [TESTS.md](TESTS.md)).

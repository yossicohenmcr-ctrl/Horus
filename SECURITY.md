...
- **Supply chain / CI:** every change is gated by a pipeline of 20 jobs — `cargo test`, `clippy` with all warnings denied, a kernel + ISO build, an alt-config build matrix (`DEBUG_SHELL=1`, `MINIMAL_SECURE=1`), fourteen headless QEMU self-tests (smoke-boot, ELF-loader + W^X, preemption, signals, process-control, async notifications, SMP, and the six-part encrypted-filesystem/libc suite), a scripted `smoke-session` integration test that drives the real ring-3 shell over serial, a byte-for-byte reproducible-build check, and a security-scan job (Semgrep, Trivy, gitleaks, cppcheck, flawfinder, `cargo-audit`) that also emits a CycloneDX SBOM. gitleaks (committed secrets, full-history) and Semgrep ERROR-severity findings are **blocking gates**; the remaining scanners are advisory. The `63`/`20` counts are enforced by `tools/check_doc_counts.sh` (see [TESTS.md](TESTS.md)).

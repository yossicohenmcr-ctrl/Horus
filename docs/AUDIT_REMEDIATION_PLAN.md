...
- **I5** — ✅ done. `tools/check_zero_deps.sh` parses `rust/Cargo.lock` and fails if it lists any package beyond `horus_shell` (added as a step in the `rust` CI job, no new job → still 21). Paired with `cargo test/clippy --locked` so a dependency added to `Cargo.toml` but not committed to the lock also fails instead of silently rewriting the lock. Guard verified: passes clean, exits 1 on an injected package.
- **Governance** — ✅ done (pragmatic posture). `CODEOWNERS` covers every security-critical path with owner review. Branch protection on `main` now requires a pull request with 1 code-owner approval **and** six passing core CI gates (rust, kernel, smoke, reproducible, tla, security) before merge. Local `make security` installs were aligned with CI's checksum-verified approach: Trivy and gitleaks are now installed from pinned release tarballs verified against SHA-256 digests (no more `curl | sh` / `@latest`). Signed-commit enforcement and admin-branch-protection were deliberately **not** enabled — they would require a working non-interactive signing method (GPG times out in this environment) and would end the maintainer's direct-to-`main` workflow; both remain available to turn on later (admins currently bypass protection, which is what keeps direct pushes working).

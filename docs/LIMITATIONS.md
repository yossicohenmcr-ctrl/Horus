...
- **Tests:** 63 Rust unit tests (capability engine, memory/refcount trust boundary, RNG and SHA-2 family vs. published vectors, the ChaCha20+HMAC AEAD, the tamper-evident audit MAC/chain, BLAKE2b + Argon2id vs. RFC 7693 / `argon2-cffi` vectors, the W^X page policy, the signal-handler-address window, FFI validation). CI runs **21 gated jobs** (`cargo test` + `clippy -D warnings`; kernel/ISO build; alt-config matrix; fourteen headless QEMU self-tests — smoke-boot, ELF/W^X, preemption, signals, process-control, async notifications, SMP, and the filesystem/libc suite: fs, fs-perms, fs-conc, fs-persist, fs-wal, fs-large, newlib; a scripted `smoke-session` integration test that drives the real ring-3 shell over serial; a reproducible-build check; a security scan + SBOM; and a TLA+ model-checking job that TLC-verifies the capability and paging invariants). The scripted-session harness (`tools/session_test.py`) is a first, deliberately small integration test — broader scenarios (W^X violations, IPC/FS round-trips) and fuzzing are still ahead.
...
| Testing | ~45% (63 unit tests + 21 CI jobs, incl. fourteen headless boot self-tests and TLA+ model checking; no deeper integration/fuzz) |
